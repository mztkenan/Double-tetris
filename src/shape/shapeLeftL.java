...
public class shapeLeftL extends absShape{              //子类
	 public shapeLeftL(mapPanel mappanel) {
			super(mappanel);
		}
    ...
    public boolean canTurn() {
    	if(state==0){             
    		int m=smallblock[0].m+1;
    		int n=smallblock[0].n+1;
    		if(m-2>-1&&map[m][n]==0&&map[m-2][n]==0&&map[m-2][n+1]==0)
    			return true;
    		else 
    			return false;
    	}
    		...
    		int m=smallblock[0].m-1;
    		int n=smallblock[0].n-1;
    		if(m+2<24&&map[m][n]==0&&map[m+2][n]==0&&map[m+2][n-1]==0)
    			return true;
    		else
    			return false;
    	}
